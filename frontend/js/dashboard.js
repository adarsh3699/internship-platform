document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('sidebar-container').innerHTML = renderSidebar(user.role);
  ...
  if (user.role === 'student') {
    ...
    document.getElementById('orgGreeting').textContent = user.companyName || user.name;
...
async function loadStudentDashboardData() {
  try {
    ...
    // Recent applications
    if (!apps.length) {
      document.getElementById('recentApplications').innerHTML = `<div class="empty-state"><div class="empty-icon">📭</div><div class="empty-title">No applications yet</div><div class="empty-desc">Start applying to opportunities!</div><button class="btn btn-primary btn-sm" onclick="window.location.href='/opportunities.html'">Browse Now</button></div>`;
    } else {
...
async function loadOrgDashboardData() {
  try {
    ...
    if (!opps.length) {
      document.getElementById('orgPosts').innerHTML = `<div class="empty-state"><div class="empty-icon">📝</div><div class="empty-title">No opportunities posted yet</div><div class="empty-desc">Post your first internship or project!</div><button class="btn btn-primary" onclick="window.location.href='/post-opportunity.html'">Post Now</button></div>`;
    } else {
