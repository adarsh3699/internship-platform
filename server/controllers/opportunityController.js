...
const getOpportunities = async (req, res) => {
  try {
    const { type, domain, mode, skills, search, page = 1, limit = 10, sort = '-createdAt' } = req.query;
    ...
    const opportunities = await Opportunity.find(query)
      .populate('organization', 'name companyName industry avatar location')
      .sort(sort)
...
const getOpportunity = async (req, res) => {
  try {
    const opportunity = await Opportunity.findById(req.params.id)
      .populate('organization', 'name companyName industry avatar location website bio companySize');
    if (!opportunity) {
...
const applyToOpportunity = async (req, res) => {
  try {
    ...
    if (opportunity.status !== 'active') {
      return res.status(400).json({ success: false, message: 'This opportunity is no longer active' });
    }
...
const getMyApplications = async (req, res) => {
  try {
    const opportunities = await Opportunity.find({ 'applicants.student': req.user.id })
      .populate('organization', 'name companyName avatar');
    const applications = opportunities.map(opp => {
      const myApp = opp.applicants.find(a => a.student.toString() === req.user.id);
      return {
        opportunity: {
          _id: opp._id,
          title: opp.title,
...
const getApplicants = async (req, res) => {
  try {
    const opportunity = await Opportunity.findById(req.params.id)
      .populate('applicants.student', 'name email skills bio education portfolio');
    if (!opportunity) return res.status(404).json({ success: false, message: 'Not found' });
    if (opportunity.organization.toString() !== req.user.id) {
      return res.status(403).json({ success: false, message: 'Not authorized' });
    }
...
const getMatchedOpportunities = async (req, res) => {
  try {
    ...
    const scored = opportunities.map(opp => {
      const matchCount = opp.skills.filter(s => student.skills.includes(s)).length;
      const matchScore = Math.round((matchCount / opp.skills.length) * 100);
...
const saveOpportunity = async (req, res) => {
  try {
    ...
    await user.save();
    res.json({ success: true, saved: !isSaved, message: isSaved ? 'Opportunity unsaved' : 'Opportunity saved' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
